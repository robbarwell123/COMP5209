...
\abstract{
  Decision Support for Organizational Change (DSOC) is an information visualization tool to help managers deal with organizational change by allowing the user to quickly see where gaps exist in an organization, understand the gaps that exist, and help the user determine how they might fill the gaps. An individual's role in an organization is multifaceted and difficult to understand, and we hope to leverage the power of visualization to make this task easier, allowing managers to make better decisions. We use emails within an organization to model the flow of information from one employee to another. Using emails as the basis of our representation of information flow provides a representation of the organization that is more concrete and up-to-date than organizational charts. 
} 
