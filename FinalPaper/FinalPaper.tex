...
\subsubsection{Data}
During the exploration of the problem space we needed to identify what data we were going to present to the user with the visualization.  Building {\color{green}upon an idea} from our colleague on information flow within an organization \textbf{{\color{Plum}Can we say a sentence or two about this previous work?  It was just an idea, not previous work.}}, we decided to use emails.  The majority of communication within a modern organization is through email or instant messaging.  We thought that analyzing email data would allow us to build an accurate and up to date model of how information flows through an organization.  
...
\subsection{Data}
Email (SMTP) header information was used to implement the value model discussed in the previous sections.  {\color{green}Extended value models are discussed later in the future work section}. \textbf{{\color{Plum}Where?}}
...
During the exploration process in finding the gap, Eric has already started to understand the size of the problem by looking at weight of the lines in the organization and process demand chart. {\textbf{\color{Plum} Are there any other ways we can determine the size of the gap using DSOC? Just checking} {\color{green}Not right now ... but maybe future work?}
...
\textbf{{\color{Plum} Needs more here. This section is meant to be a big boring summary of the paper. It's nice to finish with an optimistic line, and the one already there fulfills that need pretty well I think.}}
